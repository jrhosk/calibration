...
AntennaSolve::AntennaSolve(boost::numeric::ublas::triangular_matrix<std::complex<double>, boost::numeric::ublas::upper> & vis){
    ...
    for(unsigned i = 0; i < 10; i++) this->pGainsArray(i) = std::complex<double> {1., 0.1};

    ////this->pModelArray = boost::numeric::ublas::triangular_matrix<std::complex<double>, boost::numeric::ublas::upper> (size, size);
    this->pModelArray = boost::numeric::ublas::scalar_matrix<std::complex<double>> (10, 10, std::complex<double> {1., 1.});
}
...
void AntennaSolve::SetVis(boost::numeric::ublas::triangular_matrix<std::complex<double>, boost::numeric::ublas::upper> & vis){
    this->pObservedArray = vis;
}
...
void AntennaSolve::Fit(int n_batch, double alpha){
    ...
    for(size_t b = 0; b < n_batch; b++) {
        ...
        if(n_batch % 10 == 0) std::cout << "Gains:\t" << norm(this->pGainsArray(0)) << "\t" << norm(this->pGainsArray(1)) << std::endl;
